...
rs_int:
	...
	addl $2,%edx		/* interrupt ident. reg */ # edx 指向中断标识符寄存器
rep_int:
	xorl %eax,%eax
...
write_char:
	...
	ja 1f # 跳转到 1 
	movl proc_list(%ecx),%ebx	# wake up sleeping process # 获取当前等待的进程
...
.align 2
write_buffer_empty:
	movl proc_list(%ecx),%ebx	# wake up sleeping process # 唤醒 等待的进程
	testl %ebx,%ebx			# is there any? # 
	je 1f # 无跳转到标号 1 处
	movl $0,(%ebx) # 有 唤醒进程
1:	incl %edx 
	...
	jmp 1f
1:	jmp 1f
1:	andb $0xd,%al		/* disable transmit interrupt */ 屏蔽发送 保持寄存器空中断
	outb %al,%dx // 写入 0x3f9
	ret
