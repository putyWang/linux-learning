...
#define MAJOR_NR 2 					//软驱主设备号位 2（次设备号 = TYPE * 4 + DRIVE）
#include "blk.h"
 ...
 */
#define MAX_ERRORS 8				// 单次请求最大错误次数
...
static struct floppy_struct {
	...
	{    0, 0,0, 0,0,0x00,0x00,0x00 },	// 未使用
	{  720, 9,2,40,0,0x2A,0x02,0xDF },	// 360KB PC 软驱
	{ 2400,15,2,80,0,0x1B,0x00,0xDF },	// 1.2MB AT 软驱
	{  720, 9,2,40,1,0x2A,0x02,0xDF },	// 360KB 在 720KB 驱动器中使用 
	{ 1440, 9,2,80,0,0x2A,0x02,0xDF },	// 3.5 in 720 KB 软盘
	{  720, 9,2,40,1,0x23,0x01,0xDF },	// 360KB 在 1.2MB 驱动器中使用 
	{ 1440, 9,2,80,0,0x23,0x01,0xDF },	// 720kB in 在 1.2MB 驱动器中使用 
	{ 2880,18,2,80,0,0x1B,0x00,0xCF },	// 1.44MB 软驱
};
/*
 ...
 */
// 下面参数为传递给中断程序的全局参数
static int cur_spec1 = -1;							// 当前 spec1 参数
static int cur_rate = -1;							// 当前软驱传输速率
static struct floppy_struct * floppy = floppy_type;	// 软盘类型
...
unsigned char selected = 0;							// 当前选择使用的软驱
struct task_struct * wait_on_floppy_select = NULL; 	// 等待当前操作完成的进程

/**
 * 释放当前使用的软驱
 * @param nr 当前选择的软驱号（0-3）
*/
void floppy_deselect(unsigned int nr)
{
	if (nr != (current_DOR & 3))		// nr 不是当前软驱，打印对应错误
...
repeat:
	...
	if ((current_DOR & 3) != nr) 				// 当前进程被唤醒后，依然不是指定软驱则循环直到时指定软驱
		goto repeat;
	if (inb(FD_DIR) & 0x80) { 					// 取数字寄存器值，最高位（位7）置位则表示该软盘已被更换，关闭电动机返回1
		floppy_off(nr);
		return 1;
	}
	floppy_off(nr);
...
{
	long addr = (long) CURRENT->buffer; 	// 获取当前缓冲区地址
...
static void output_byte(char byte)
{
	...
	for(counter = 0 ; counter < 10000 ; counter++) {
		// 如果软盘控制器处于就绪状态且 STATUS_DIR=0（CPU->FDC）时，则向数据端口输出指定字节
	...
	reset = 1;	// 循环 1 万次结束还不能成功发送，则置复位标志，并打印错误信息
...
static int result(void)
{
	...
	for (counter = 0 ; counter < 10000 ; counter++) {
		...
		if (status == STATUS_READY) 							// 软盘控制器只是处于就绪状态，返回 i
			return i;
		if (status == (STATUS_DIR|STATUS_READY|STATUS_BUSY)) {	// 软盘控制器处于 STATUS_DIR=1（FDC->CPU）、就绪以及忙状态时，则读取相关参数
			if (i >= MAX_REPLIES)								// 执行结果已经完成时，结束循环
				break;
			reply_buffer[i++] = inb_p(FD_DATA); 				// 保存读取参数
		}
	}
	reset = 1;													// 超时重新设置reset 等待下次重试
...
{
	CURRENT->errors++;					// 当前错误次数 +1
	if (CURRENT->errors > MAX_ERRORS) {	// 大于最大了允许出错次数，结束当前请求，返回出错
		floppy_deselect(current_drive);
		end_request(0);
	}
	if (CURRENT->errors > MAX_ERRORS/2)	// 大于最大出错次数一半时，进行复位操作
		reset = 1;
	else
		recalibrate = 1;				// 未大于一半则只进行重新校正操作
}	
...
static void rw_interrupt(void)
{
	...
	floppy_deselect(current_drive); // 释放当前软驱，在执行其他软盘请求项
	end_request(1);
	do_fd_request();
}
...
/* sense drive status */
	...
	current_track = ST1;		// 将 ST1 设置为当前磁道
	setup_rw_floppy();			// 设置DMA 并 输出软盘操作命令和参数
}
...
void do_fd_request(void)
{
	...
	add_timer(ticks_to_floppy_on(current_drive),&floppy_on_interrupt);	// 添加用于指定驱动器到能正常运行所需延迟时间的定时器（定时结束调用 floppy_on_interrupt 函数）
}
